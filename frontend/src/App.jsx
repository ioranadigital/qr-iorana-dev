...
async function apiFetch(path, options = {}) {
  ...
  if (!res.ok) { const err = await res.json().catch(() => ({ error: res.statusText })); throw new Error(err.error || "Error desconocido"); }
  return res.json();
}
...
function QRImage({ value, size = 80, fgColor = "#000000", bgColor = "#ffffff" }) {
  ...
  return <div style={{ width: size, height: size, background: bgColor, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><img src={src} width={innerSize} height={innerSize} style={{ display: "block" }} alt="QR" /></div>;
}
...
function ExportModal({ qr, onClose }) {
  ...
  const friendlyUrl = qr.slug && qr.slugDomain ? `https://${qr.slugDomain}/${qr.slug}` : null;
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
      <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1.5rem", width: 360, maxWidth: "92vw" }}>
        ...
        <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "10px 12px", marginBottom: 14 }}>
          <p style={{ fontSize: 11, color: "var(--color-text-tertiary)", margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "0.05em" }}>URL en el QR</p>
          <p style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--color-text-secondary)", margin: 0, wordBreak: "break-all" }}>{url}</p>
          {friendlyUrl && <>
            <p style={{ fontSize: 11, color: "var(--color-text-tertiary)", margin: "8px 0 4px", textTransform: "uppercase", letterSpacing: "0.05em" }}>URL amigable (vista por el usuario)</p>
            <p style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--color-text-primary)", margin: 0, wordBreak: "break-all" }}>{friendlyUrl}</p>
          </>}
        </div>
      ...
      <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "2rem", width: 340, maxWidth: "92vw" }}>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          <div style={{ width: 48, height: 48, background: "#1a1a2e", borderRadius: 12, margin: "0 auto 12px", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="8" height="8" rx="1" fill="white"/><rect x="5" y="5" width="4" height="4" fill="#1a1a2e"/><rect x="13" y="3" width="8" height="8" rx="1" fill="white"/><rect x="15" y="5" width="4" height="4" fill="#1a1a2e"/><rect x="3" y="13" width="8" height="8" rx="1" fill="white"/><rect x="5" y="15" width="4" height="4" fill="#1a1a2e"/><rect x="13" y="13" width="3" height="3" fill="white"/><rect x="18" y="13" width="3" height="3" fill="white"/><rect x="13" y="18" width="3" height="3" fill="white"/><rect x="18" y="18" width="3" height="3" fill="white"/></svg>
          </div>
        ...
        <button onClick={handleSubmit} disabled={loading || !username || !password} style={solidBtn(loading || !username || !password, "#1a1a2e", true)}>{loading ? "Entrando…" : "Entrar"}</button>
      </div>
    </div>
  );
}
...
// ── Slug field ────────────────────────────────────────
function SlugField({ slug, setSlug, slugDomain, setSlugDomain, domains, compact = false }) {
  const friendlyUrl = slug && slugDomain ? `https://${slugDomain}/${slug}` : null;
  return (
    <div>
      <label style={{ ...labelStyle, display: "flex", alignItems: "center", gap: 6 }}>
        URL amigable
        <span style={{ fontSize: 11, background: "var(--color-background-info)", color: "var(--color-text-info)", borderRadius: 4, padding: "1px 6px" }}>opcional</span>
      </label>
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 6 }}>
        ...
        <input value={slug} onChange={e => setSlug(e.target.value.replace(/\s+/g, "-").toLowerCase())} placeholder="pymes/todo-para-tu-negocio" style={{ flex: 1, fontSize: 13 }} />
      </div>
      {friendlyUrl && (
        <div style={{ display: "flex", alignItems: "center", gap: 6, background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "6px 10px" }}>
          <span style={{ fontSize: 11, color: "var(--color-text-tertiary)", flexShrink: 0 }}>Vista previa:</span>
          <span style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--color-text-primary)", wordBreak: "break-all" }}>{friendlyUrl}</span>
        </div>
      )}
      {!slug && (
        <p style={{ fontSize: 11, color: "var(--color-text-tertiary)", margin: "4px 0 0" }}>
          Si lo dejas vacío, el QR usará la URL corta estándar ({BASE_URL}ID).
        </p>
      )}
    </div>
  );
...
const solidBtn = (disabled = false, bg = "#1a1a2e", full = false) => ({ width: full ? "100%" : undefined, flex: full ? undefined : 1, padding: "9px 16px", fontSize: 13, fontWeight: 500, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.45 : 1, borderRadius: "var(--border-radius-md)", border: "none", background: disabled ? "var(--color-background-secondary)" : bg, color: disabled ? "var(--color-text-tertiary)" : "#ffffff" });
...
export default function App() {
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "0 1rem 2rem", fontFamily: "var(--font-sans)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1.25rem 0 1rem", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 34, height: 34, background: "#1a1a2e", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="8" height="8" rx="1" fill="white"/><rect x="5" y="5" width="4" height="4" fill="#1a1a2e"/><rect x="13" y="3" width="8" height="8" rx="1" fill="white"/><rect x="15" y="5" width="4" height="4" fill="#1a1a2e"/><rect x="3" y="13" width="8" height="8" rx="1" fill="white"/><rect x="5" y="15" width="4" height="4" fill="#1a1a2e"/><rect x="13" y="13" width="3" height="3" fill="white"/><rect x="18" y="13" width="3" height="3" fill="white"/><rect x="13" y="18" width="3" height="3" fill="white"/><rect x="18" y="18" width="3" height="3" fill="white"/></svg>
          </div>
          <div>
      ...
      {/* Error */}
      {error && <div style={{ background: "var(--color-background-danger)", border: "0.5px solid var(--color-border-danger)", borderRadius: "var(--border-radius-md)", padding: "10px 14px", margin: "12px 0", fontSize: 13, color: "var(--color-text-danger)", display: "flex", justifyContent: "space-between" }}>{error}<button onClick={() => setError(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--color-text-danger)", fontSize: 16, lineHeight: 1 }}>×</button></div>}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 20, alignItems: "start" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div>
                ...
                <input value={destUrl} onChange={e => setDestUrl(e.target.value)} placeholder="https://ejemplo.com" style={{ width: "100%", boxSizing: "border-box" }} />
              </div>
              <SlugField slug={slug} setSlug={setSlug} slugDomain={slugDomain} setSlugDomain={setSlugDomain} domains={domains} />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {visibleQrs.map(qr => (
            <div key={qr.id} style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1rem 1.25rem", display: "grid", gridTemplateColumns: "auto 1fr", gap: 14, alignItems: "start" }}>
              <div style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, background: qr.bgColor || "#fff", cursor: "pointer", overflow: "hidden" }} onClick={() => setExportTarget(qr)}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                {editId === qr.id ? (
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <button onClick={() => { setEditId(qr.id); setEditUrl(qr.destUrl); setEditLabel(qr.label); setEditFg(qr.fgColor); setEditBg(qr.bgColor); setEditSlug(qr.slug || ""); setEditSlugDomain(qr.slugDomain || domains[0]); }} style={{ fontSize: 12, padding: "5px 12px", border: "0.5px solid var(--color-border-secondary)", borderRadius: "var(--border-radius-md)", cursor: "pointer", background: "var(--color-background-primary)", color: "var(--color-text-primary)", whiteSpace: "nowrap" }}>Editar</button>
                    <button onClick={() => setExportTarget(qr)} style={{ fontSize: 12, padding: "5px 12px", border: "0.5px solid var(--color-border-secondary)", borderRadius: "var(--border-radius-md)", cursor: "pointer", background: "var(--color-background-primary)", color: "var(--color-text-primary)", whiteSpace: "nowrap" }}>Exportar</button>
                    <button onClick={() => setDeleteTarget(qr)} style={{ fontSize: 12, padding: "5px 10px", border: "0.5px solid var(--color-border-danger)", borderRadius: "var(--border-radius-md)", cursor: "pointer", background: "var(--color-background-primary)", color: "var(--color-text-danger)", whiteSpace: "nowrap" }}>Eliminar</button>
                  </div>
                )}
              </div>
            </div>
